  ...
  return (
    <div className="space-y-6">
      ...
      <textarea
        ...
        className="w-full h-80 px-4 py-3 rounded-xl bg-secondary border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary/50 resize-none font-mono leading-relaxed"
      />
      ...
      <div className="flex items-center gap-6 text-sm">
        ...
        <span className="flex items-center gap-2 text-muted-foreground">
          <Clock className="w-4 h-4" /> ~{estimatedDuration} min video
        </span>
        {wordCount > 0 && wordCount < 150 && (
          <span className="flex items-center gap-2 text-warning text-xs">
            <AlertTriangle className="w-3 h-3" /> Script may be too short for engagement
          </span>
        )}
      </div>
    </div>
