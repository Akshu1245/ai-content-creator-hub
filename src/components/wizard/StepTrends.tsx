...
import { Eye, Clock, ThumbsUp, BarChart3, TrendingUp } from "lucide-react";
...
const StepTrends = ({ data }: Props) => {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-display font-semibold text-foreground mb-1">Trend Intelligence</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
        {[
          ...
          <div key={m.label} className="surface p-3 text-center">
            ...
            <div className="text-xs font-label text-muted-foreground mt-0.5">{m.label.toUpperCase()}</div>
          </div>
        ))}
      </div>

      <div className="surface-raised p-5">
        <h3 className="text-sm font-display font-semibold text-foreground mb-4">Search Interest (30 days)</h3>
      ...
      <div>
        <h3 className="text-sm font-display font-semibold text-foreground mb-3">Top Competing Videos</h3>
        <div className="space-y-2">
          {mockCompetitors.map((c, i) => (
            <div key={i} className="surface p-3 flex items-center justify-between surface-hover">
