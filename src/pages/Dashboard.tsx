...
const getStatusStyle = (status: string) => {
  switch (status) {
    case "Live": return "text-primary bg-primary/8 border-primary/20";
    case "Generating": return "text-accent bg-accent/8 border-accent/20";
    case "Scheduled": return "text-golden bg-golden/8 border-golden/20";
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <span className="font-label text-muted-foreground block mb-1">OVERVIEW</span>
          ...
          {stats.map((stat) => (
            <div key={stat.label} className="surface-raised p-5 surface-hover">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[11px] font-label text-muted-foreground">{stat.label.toUpperCase()}</span>
              </div>
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-lg font-display text-foreground">Active Projects</h2>
              <span className="text-[11px] font-label text-muted-foreground">{projects.length} PROJECTS</span>
            </div>
              ...
              <Link key={project.id} to={`/project/${project.id}`}>
                <div className="surface-raised p-5 surface-hover cursor-pointer group">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-display text-foreground group-hover:text-primary transition-colors">{project.name}</h3>
                      <p className="text-xs text-muted-foreground mt-0.5">{project.niche} · {project.videos} videos</p>
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold border ${getStatusStyle(project.status)}`}>{project.status}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground truncate mr-4 text-xs italic">{project.lastVideo}</span>
                    <div className="flex items-center gap-4 shrink-0 text-xs">
                      <span className="flex items-center gap-1 font-mono text-muted-foreground">
                        <Eye className="w-3 h-3" /> {project.views}
                      </span>
                      <span className={`font-mono font-semibold ${project.compliance >= 80 ? "text-primary" : "text-golden"}`}>
                        {project.compliance}
                      </span>
                    </div>
                  </div>
          ...
          <div className="space-y-5">
            <div>
              <h2 className="text-lg font-display text-foreground mb-3">Recent Activity</h2>
              <div className="surface-raised p-5 space-y-3.5">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <Link key={action.to} to={action.to}>
                    <div className="surface-raised p-4 surface-hover flex items-center justify-between cursor-pointer">
